(function() {
	...
	var eventContent = {
		"event-kids-jiu-jitsu": `
		<div class="cd-schedule-modal__event-info">
			<h3>Kids Jiu-Jitsu</h3>
			<p>This class focuses on basic Jiu-Jitsu skills tailored for children. It promotes physical fitness and discipline.</p>
		</div>
		`,
		"event-kick-boxing-beginner": `
		<div class="cd-schedule-modal__event-info">
			<h3>Kick Boxing (Beginner)</h3>
			<p>Beginner-friendly kickboxing classes that teach you the fundamentals of the sport in a supportive environment.</p>
		</div>
		`,
		"event-kick-boxing-advanced": `
		<div class="cd-schedule-modal__event-info">
			<h3>Kick Boxing (Advanced)</h3>
			<p>Advanced techniques for experienced kickboxers looking to refine their skills and competitive edge.</p>
		</div>
		`,
		"event-adult-jiu-jitsu": `
		<div class="cd-schedule-modal__event-info">
			<h3>Adult Jiu-Jitsu</h3>
			<p>Coming soon.</p>
		</div>
		`
	};
	...
	ScheduleTemplate.prototype.placeEvents = function() {
		...
		Util.removeClass(this.element, 'cd-schedule--loading');
		scheduleTemplateArray.forEach(function(template) {
			template.updateScheduleHeight(); // Adjust schedule height based on the events
		});
	};
	...
	ScheduleTemplate.prototype.loadEventContent = function(content) {
		var self = this;
		// Check if the content exists in the eventContent object
		if (eventContent[content]) {
			console.log("Displaying content for:", content); // Debugging: confirm the content being displayed
			self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML = eventContent[content];
			Util.addClass(self.modal, 'cd-schedule-modal--content-loaded');
		} else {
			console.error("No content found for:", content); // Error handling: content not found
			self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML = '<p>No content available.</p>';
		}
	};
	...
	ScheduleTemplate.prototype.updateScheduleHeight = function() {
		var lastEvent = this.singleEvents[this.singleEvents.length - 1]; // get the last event
		var lastAnchor = lastEvent.getElementsByTagName('a')[0];
		var end = getScheduleTimestamp(lastAnchor.getAttribute('data-end'));
	
		var slotHeight = this.topInfoElement.offsetHeight;
		var totalHeight = slotHeight * (end - this.timelineStart) / this.timelineUnitDuration;
	
		this.element.getElementsByClassName('cd-schedule__events')[0].style.height = totalHeight + 'px'; // Adjust the height directly
	};
